...
#
# It therefore needs `dist/<arch>/mcp-iap` in the context, which a fresh
# checkout does not have. To build it by hand:
#
#   cargo build --release --target x86_64-unknown-linux-musl
#   mkdir -p dist/amd64 && cp target/x86_64-unknown-linux-musl/release/mcp-iap dist/amd64/
#   docker build -t mcp-iap .
...
ARG TARGETARCH
COPY dist/$TARGETARCH/mcp-iap /rootfs/usr/local/bin/mcp-iap
...
RUN chmod 0755 /rootfs/usr/local/bin/mcp-iap \
    && mkdir -p /rootfs/etc/mcp-iap /rootfs/var/lib/mcp-iap \
    && chown -R 65532:65532 /rootfs/var/lib/mcp-iap \
    && chmod 0700 /rootfs/var/lib/mcp-iap
...
LABEL org.opencontainers.image.title="mcp-iap" \
      ...
      org.opencontainers.image.source="https://github.com/vpetersson/mcp-iap" \
...
ENV IAP_CONFIG=/etc/mcp-iap/iap.toml
...
WORKDIR /var/lib/mcp-iap
USER 65532:65532
ENTRYPOINT ["/usr/local/bin/mcp-iap"]
