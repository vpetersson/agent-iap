...
For a service that already has a profile, step 2 is one command — the base URL,
the credential scheme and the scopes, all worked out — see
[§ Profiles](#profiles):
...
That is `profile add github --as gh`, because "add an upstream" is what somebody
sets out to do, rather than having to know profiles exist first. `--access`,
`--var`, `--grant`, `--agent` and `--dry-run` mean what they do below; the
credential flags do not, since the profile supplies the scheme, so anything
beyond `--secret` is refused rather than ignored. In the console, `n` on the
upstreams pane opens the same picker (§ The approval console).
